...
class ProductPage {
  ...
  async createProduct(data) {
    ...
    // const barcodeField = await this.barcodeField();
    // await barcodeField.clearValue();
    // await barcodeField.setValue(data.barcode);
    //  await key.AllureScreenshot('Entered Barcode');
    ...
    await key.AllureScreenshot("product creation  Message Displayed");

    if (message.includes("bereits in Gebrauch")) {
      console.warn("SKU already exists. Try a unique SKU.");
      await key.AllureScreenshot("SKU already exists");
    } else {
